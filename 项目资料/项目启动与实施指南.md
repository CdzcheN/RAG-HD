...
| `HF_HUB_DISABLE_XET` | 禁用 Xet 大文件通道（**本机必设**，见 §1.3 说明） | `export HF_HUB_DISABLE_XET=1` | `$env:HF_HUB_DISABLE_XET="1"` |
...
**大文件卡在 Xet 端点时怎么办（本机实测，2026-09）**：hf-mirror 只代理元数据，模型权重这类 Xet 存储的大文件会被 302 到 `cas-bridge.xethub.hf.co`；本机实测该端点仅约 **0.2 MB/s** 且频繁 `Read timed out`（942 MB 的 `model.safetensors` 按此速度需约 70 分钟）。处理办法：

1. **先禁用 Xet、放宽超时**（见 §1.1 的 `HF_HUB_DISABLE_XET`），回退到可续传的经典单流下载；
2. **仍不理想时用 ModelScope 备源**——`scripts/prefetch_assets.py` 已内置回退：HF 通道失败会自动从 ModelScope（实测约 **5 MB/s**，快约 20 倍）下载同一份文件，按 HF 声明的 sha256（LFS 文件）或 git blob sha1（普通文件）校验后放进**同一个 HF 缓存目录**，因此 `transformers` / `sentence-transformers` 侧代码无需任何改动；
3. **判断是否成功**：`refs/main` 与 `snapshots/<commit>/` 下的文件链接齐全即表示缓存自洽，随后可设 `HF_HUB_OFFLINE=1` 完全离线复现。

> 备源只影响下载通道，不影响可复现性：ModelScope 上的权重与 HF 同名文件 sha256 一致（脚本会逐个校验，不一致就报错退出）。
...
- 工作区 `data/raw/` 下的本地副本：HotpotQA 的三份 parquet 与 HF 声明哈希一致，`prefetch_assets.py --data` 会在网络不畅时**直接复用它们补齐缓存**（零下载）；`scripts/download_data.sh`、`scripts/verify_data.py` 作为离线备选与历史校验记录保留（前者是 bash 脚本，Windows 需 WSL / Git Bash）。

**大文件卡在 Xet 时怎么办（与 §1.3 同源问题）**：HF datasets 的 parquet 同样在 Xet 存储上，本机实测经镜像只有约 **0.13 MB/s**，且 `datasets` 的下载会长时间停在 **0 字节**（不是超时失败，而是不推进，表现为长时间无任何进度）。`prefetch_assets.py --data` 的处理顺序为：

1. **本地文件优先**：若 `data/raw/` 已备有该数据集的文件且哈希与 HF 声明一致，直接硬链接进缓存，零下载（实测 HotpotQA 三份 parquet 全部命中）；
2. **HF 通道**：正常走 `HF_ENDPOINT` 镜像，并把各 split 条数打印出来与上表核对；
3. **回退**：HF 通道失败时按「本地 → ModelScope → hf-mirror」逐文件补齐；`.gitattributes`、`README.md` 这类纯元数据缺失只告警、不影响加载。

补齐过程中只取**目标配置**的文件（如 `distractor/`），不会连带下载 `fullwiki/`。补齐后 `load_dataset(...)` 调用无需任何改动。

> **离线复现靠 arrow 缓存**：`datasets` 5.x 在纯离线模式下仍需 Hub API 解析仓库，因此请先联网成功加载一次（生成 `~/.cache/huggingface/datasets/` 下的 arrow 缓存），再设 `HF_HUB_OFFLINE=1`。
